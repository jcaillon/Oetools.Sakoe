...
namespace csdeployer.Core {
    ...
    [Serializable]
    [XmlRoot("Config")]
    public class CsDeployerInterfaceXml {

        #region static

        internal static CsDeployerInterfaceXml Load(string path) {
            CsDeployerInterfaceXml interfaceXml;
            var serializer = new XmlSerializer(typeof(CsDeployerInterfaceXml));
            using (var reader = new StreamReader(path)) {
                interfaceXml = (CsDeployerInterfaceXml)serializer.Deserialize(reader);
            }
            return interfaceXml;
        }

        internal static void Save(CsDeployerInterfaceXml interfaceXml, string path) {
            var serializer = new XmlSerializer(typeof(CsDeployerInterfaceXml));
