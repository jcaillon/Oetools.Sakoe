...
namespace Oetools.Sakoe.Command.Oe {
    ...
    internal class DumpDfDatabaseCommand : ADatabaseSingleConnectionCommand {
        ...
        protected override int ExecuteCommand(CommandLineApplication app, IConsole console) {
            using (var ope = GetAdministrator()) {
                ope.DumpSchemaDefinition(GetSingleDatabaseConnection(), DumpFilePath, TableName);
            }
            return 0;
        }
    }
    ...
    internal class DumpIncrementalDfDatabaseCommand : ADatabaseCommand {
        ...
        [Required]
        [LegalFilePath]
        [Argument(2, "<dump-file>", "Path to the output incremental " + UoeDatabaseLocation.SchemaDefinitionExtension + " file.")]
        public string DumpFilePath { get; set; }
...
", CommandOptionType.SingleValue)]
        ...
        protected override int ExecuteCommand(CommandLineApplication app, IConsole console) {
            using (var ope = GetAdministrator()) {
                ope.DumpIncrementalSchemaDefinitionFromDatabases(new [] { ope.GetDatabaseConnection(new UoeDatabaseLocation(NewDatabasePath)), ope.GetDatabaseConnection(new UoeDatabaseLocation(OldDatabasePath)) }, DumpFilePath, RenameFilePath);
            }
            return 0;
        }
    }
...
", CommandOptionType.SingleValue)]
        ...
        protected override int ExecuteCommand(CommandLineApplication app, IConsole console) {
            using (var ope = GetAdministrator()) {
                ope.DumpIncrementalSchemaDefinition(OldDfPath?.ToAbsolutePath(), NewDfPath?.ToAbsolutePath(), DumpFilePath, RenameFilePath);
            }
            return 0;
        }
    }
